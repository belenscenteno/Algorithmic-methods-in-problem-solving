...
protected:
    ...
    struct TreeNode {
        ...
        TreeNode(clave_valor const& e, Link i = nullptr, Link d = nullptr,
                 int alt = 1, int tamizq = 1) : cv(e), iz(i), dr(d), altura(alt), tam_i(tamizq) {}
    };
...
public:
    ...
    int findK(int k ) {
        ...
        else return findK2(this->raiz,k);
    }
    
protected:
    
    int findK2(Link r, int k){
        if(r == nullptr) return -1;
        
        if(r->tam_i == k)
            return r->cv.clave;
        else if (r->tam_i < k)
            return findK2(r->dr, k - r->tam_i);
        else return findK2(r->iz, k);
    }
